...
void TextureCacheCommon::DumpTextureDirect(u32 texAddr, u16 w, u16 h, u16 fmt, u32 clutAddr, u16 clutFmt, u32 clutEntries, const std::string& outDirOverride) {
	...
	if (!Memory::IsValidAddress(texAddr)) return;
	if (!Memory::IsValidAddress(texAddr + pixelSize - 1)) return;
	if (clutAddr && !Memory::IsValidAddress(clutAddr)) return;

	const u8* texData = Memory::GetPointerUnchecked(texAddr);
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			...
			if (fmt == GE_TFMT_CLUT8 && clutData) {
				u8 ci = texData[idx];
				if (clutFmt == 3) pixel = ((u32*)clutData)[ci];
				else if (clutFmt == 0) { u16 c = ((u16*)clutData)[ci]; pixel = 0xFF000000 | (((c >> 11) & 0x1F) << 19) | (((c >> 5) & 0x3F) << 10) | ((c & 0x1F) << 3); }
				else if (clutFmt == 1) { u16 c = ((u16*)clutData)[ci]; pixel = ((c >> 15) ? 0xFF000000 : 0) | (((c >> 10) & 0x1F) << 19) | (((c >> 5) & 0x1F) << 11) | ((c & 0x1F) << 3); }
				else if (clutFmt == 2) { u16 c = ((u16*)clutData)[ci]; pixel = (((c >> 12) & 0xF) * 17 << 24) | (((c >> 8) & 0xF) * 17 << 16) | (((c >> 4) & 0xF) * 17 << 8) | ((c & 0xF) * 17); }
			}
			else if (fmt == GE_TFMT_CLUT4 && clutData) {
				u8 ci = (texData[idx / 2] >> ((idx & 1) * 4)) & 0xF;
				if (clutFmt == 3) pixel = ((u32*)clutData)[ci];
				else if (clutFmt == 0) { u16 c = ((u16*)clutData)[ci]; pixel = 0xFF000000 | (((c >> 11) & 0x1F) << 19) | (((c >> 5) & 0x3F) << 10) | ((c & 0x1F) << 3); }
				else if (clutFmt == 1) { u16 c = ((u16*)clutData)[ci]; pixel = ((c >> 15) ? 0xFF000000 : 0) | (((c >> 10) & 0x1F) << 19) | (((c >> 5) & 0x1F) << 11) | ((c & 0x1F) << 3); }
				else if (clutFmt == 2) { u16 c = ((u16*)clutData)[ci]; pixel = (((c >> 12) & 0xF) * 17 << 24) | (((c >> 8) & 0xF) * 17 << 16) | (((c >> 4) & 0xF) * 17 << 8) | ((c & 0xF) * 17); }
			}
			else if (fmt == GE_TFMT_8888) {
				pixel = ((u32*)texData)[idx];
			}
			else if (fmt == GE_TFMT_5650) {
				u16 c = ((u16*)texData)[idx]; pixel = 0xFF000000 | (((c >> 11) & 0x1F) << 19) | (((c >> 5) & 0x3F) << 10) | ((c & 0x1F) << 3);
			}
			else if (fmt == GE_TFMT_5551) {
				u16 c = ((u16*)texData)[idx]; pixel = ((c >> 15) ? 0xFF000000 : 0) | (((c >> 10) & 0x1F) << 19) | (((c >> 5) & 0x1F) << 11) | ((c & 0x1F) << 3);
			}
			else if (fmt == GE_TFMT_4444) {
				u16 c = ((u16*)texData)[idx]; pixel = (((c >> 12) & 0xF) * 17 << 24) | (((c >> 8) & 0xF) * 17 << 16) | (((c >> 4) & 0xF) * 17 << 8) | ((c & 0xF) * 17);
			}
	...
	Path outDir = outDirOverride.empty() ? GetSysDirectory(DIRECTORY_TEXTURES) / gameID / "new" : Path(outDirOverride);
	...
	if (pngf) {
		...
		png_set_IHDR(png, pinfo, w, h, 8, PNG_COLOR_TYPE_RGBA,
			PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
		png_write_info(png, pinfo);
		for (int y = 0; y < (int)h; y++) {
			png_write_row(png, (png_bytep)(decoded.data() + y * w));
		}
...
void TextureCacheCommon::ApplyTexture(bool doBind) {
	...
	if (nextNeedsRebuild_) {
		...
		if (nextNeedsRehash_) {
			...
			if (g_Config.bDumpAllTexturesOnLoad && replacer_.SaveEnabled()) {
				ReplacedTextureDecodeInfo replacedInfo;
				replacedInfo.cachekey = entry->CacheKey();
				replacedInfo.hash = entry->fullhash;
				replacedInfo.addr = entry->addr;
				replacedInfo.isFinal = true;
				replacedInfo.isVideo = false;
				replacedInfo.fmt = Draw::DataFormat::R8G8B8A8_UNORM;
				if (replacer_.WillSave(replacedInfo)) {
					entry->needsDecodeForDump = true;
				}
			}
...
bool TextureCacheCommon::PrepareBuildTexture(BuildTexturePlan &plan, TexCacheEntry *entry) {
	...
	if (entry->status & TexCacheEntry::TexStatus::STATUS_CLUT_GPU) {
		...
		plan.scaleFactor = 1;
		plan.saveTexture = entry->needsDecodeForDump;  // Can't yet save these properly.
		canReplace = false;
	} else {
...
void TextureCacheCommon::LoadTextureLevel(TexCacheEntry &entry, uint8_t *data, size_t dataSize, int stride, BuildTexturePlan &plan, int srcLevel, Draw::DataFormat dstFmt, TexDecodeFlags texDecFlags) {
	...
	if (plan.doReplace) {
		...
		// Force-dump every decoded texture
		if (g_Config.bSaveNewTextures && srcLevel == 0 && w > 0 && h > 0) {
			u32 texHash = StableQuickTexHash(Memory::GetPointerUnchecked(texaddr), (textureBitsPerPixel[tfmt] * bufw * h) / 8);
			u32 clutH = clutHash_ ^ gstate.clutformat;
			std::string gid = g_paramSFO.GetDiscID();
			std::string dir = (GetSysDirectory(DIRECTORY_TEXTURES) / gid / "new").ToString();
			File::CreateFullPath(Path(dir));
			char fn[512];
			snprintf(fn, sizeof(fn), "%s\\%08x%08x%08x.png", dir.c_str(), texaddr, clutH, texHash);
			FILE* pf = fopen(fn, "wb");
			if (pf) {
				png_structp p = png_create_write_struct(PNG_LIBPNG_VER_STRING, 0, 0, 0);
				png_infop pi = png_create_info_struct(p);
				png_init_io(p, pf);
				png_set_IHDR(p, pi, w, h, 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
				png_write_info(p, pi);
				for (int y = 0; y < h; y++)
					png_write_row(p, (png_bytep)((u32*)pixelData + y * (decPitch / 4)));
				png_write_end(p, pi);
